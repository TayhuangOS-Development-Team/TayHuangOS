...
typedef struct {
    ...
    byte root_directory[8192]; //根目录数据
} FAT32_CONTEXT;
...
dword get_fat32_entry(void *context, dword last) { //获取FAT32项
...
void __loadfile(void *context, dword clus, void *dst) {
    FAT32_CONTEXT *_context = (FAT32_CONTEXT*)context;
    //起始簇号为2
...
void get_context(int selector, void **__context) { //获取上下文
    FAT32_CONTEXT *context = (FAT32_CONTEXT*)__context;
    char boot[1024];
...
dword get_file_start_clus(void *context, const char *name) {
    if (strlen(name) != 11)
        return -1;
    FAT32_CONTEXT *_context = (FAT32_CONTEXT*)context;
    char rdname[12] = "";
...
char *convert(const char *name, char *buffer) { //装换为FAT32格式名称
    char *backup = buffer;
    int cnt = 0;
    while (*name != '.') {
        *buffer = toupper(*name);
...
bool loadfile(void *context, const char *name, void *dst) {
    FAT32_CONTEXT *_context = (FAT32_CONTEXT*)context;
    char _name[12];
    if (convert(name, _name) == NULL) { //转换名称
        return false;
    }
    dword clus = get_file_start_clus(context, _name); //获取起始扇区
    if (clus == -1) {
        return false;
    }
    __loadfile(_context, clus, dst); //加载
    return true;
}